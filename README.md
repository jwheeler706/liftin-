...
Add a new workout page by copying one of the existing workout HTML files in `workouts/` and editing the `window.liftWorkoutConfig` object near the bottom. Then add a card for it in `index.html`.

The shared behavior lives in `assets/js/workout-app.js`, so new workout pages get timers, logging, backup swaps, skips, added sets, and dynamic finishers automatically.
